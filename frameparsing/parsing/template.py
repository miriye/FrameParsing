...
class Template(str):
    ...
    @property
    def hash(self) -> str:
        """String in "hash" format. e.g: "frame####.png"."""
        return self._parser.translate("hash")
    ...
    @property
    def width(self) -> int:
        """The fill width of the framecode in this template."""
    ...
    def matches(self, string: str, strict: bool = True) -> bool:
        ...
        Args:
            ...
            strict: If True, match template exactly from start to end of string. If False, 
                match anywhere in the string. Defaults to True. 

        Returns:
            True if the string matches this template, False if not.
        """
        if isinstance(string, Path):
            string = str(string)

        # Check if string matches template (works for digits)
        ...
        # Check if template would be the same (works for any framecode)
        try:
            other_template = Template(string)
            return self == other_template
        except ValueError:
            return False
